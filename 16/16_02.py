...
def process_values(values, type):
...
def parse_package(offset, bin_input):
    ...
    if type_id != 4:
        ...
        if length_type_id == 0:
            ...
            tmp_values = []
            # Evaluate inner packages by package type
            for i in range(packages_done):
                tmp_values.append(literal_values.pop())
            # Reverse list to fix order
            # Order is being defined by using a stack
            tmp_values.reverse()

            literal_values.append(process_values(tmp_values, types.pop()))
...
print(f"Solution: {literal_values}")
